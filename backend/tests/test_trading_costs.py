...
def test_default_backtest_cost_is_explicit_and_nonzero() -> None:
    config = default_backtest_cost_config()

    assert config.fee_rate_per_side > 0
    assert config.slippage_rate_per_side > 0
    assert roundtrip_cost_rate(config) == pytest.approx(0.001)
