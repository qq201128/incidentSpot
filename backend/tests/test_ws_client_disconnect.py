...
from app.services.ws_client_disconnect import (
    CLIENT_WS_GONE_EXC,
    ClientDisconnected,
...
def test_client_ws_gone_includes_client_disconnected() -> None:
    ...
    if UvicornClientDisconnected is not None:
        assert UvicornClientDisconnected in CLIENT_WS_GONE_EXC
...
def test_index_rest_fallback_reraises_uvicorn_client_disconnected() -> None:
    if UvicornClientDisconnected is None:
        return

    async def run() -> None:
        client_ws = AsyncMock()
        client_ws.send_json.side_effect = UvicornClientDisconnected()
        row = {
            "openTime": 1,
            "open": "1",
            "high": "1",
        ...
        with patch(
            ...
            with pytest.raises(UvicornClientDisconnected):
                await send_index_rest_fallback(client_ws, "BTCUSDT", "10m", "test")
