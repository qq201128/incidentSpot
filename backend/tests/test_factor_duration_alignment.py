...
def test_backtest_duration_frame_samples_completed_duration_entries() -> None:
    ...
    assert list(out["open_time"].head(3)) == [0, TEN_MINUTES * ONE_MINUTE_MS, 2 * TEN_MINUTES * ONE_MINUTE_MS]
    ...
    assert out["fwd_ret"].iloc[0] == expected.iloc[0]
...
def _frame(rows: int) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
            "open_time": np.arange(rows) * TEN_MINUTES * ONE_MINUTE_MS,
            "close": 100.0 + index,
            "factor_a": index,
        }
    )
