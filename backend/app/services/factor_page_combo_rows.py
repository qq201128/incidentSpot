...
def combo_cache_summary(row: dict[str, Any], symbol: str, duration: str) -> dict[str, Any]:
    ...
    return {
        ...
        "avgTradesPerDay": row.get("avgTradesPerDay"),
        "sampleDays": row.get("sampleDays"),
        "icMean": row.get("icMean"),
        "ir": row.get("ir"),
        "maxDrawdown": row.get("maxDrawdown"),
        "backtestValid": row.get("backtestValid"),
        "backtestInvalidReason": row.get("backtestInvalidReason"),
