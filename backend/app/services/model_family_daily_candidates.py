...
def model_family_daily_candidate_report(
    ...
    *,
    ...
    rows = [_family_status_payload(family, symbol, duration, status_loader, lifecycle) for family in families]
...
def _family_status_payload(
    ...
    paper_live = lifecycle.get(family) or {}
    paper_status = paper_live.get("paperLiveStatus") or admission.get("status")
    return {
        "modelFamily": family,
        ...
        "paperLiveStatus": paper_status,
        "paperLiveReason": paper_live.get("reason") or admission.get("reason"),
