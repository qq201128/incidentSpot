...
def _threshold_returns(frame: pd.DataFrame, hit: ComboHit) -> pd.Series:
    ...
    returns = signal.set_axis(frame.index) * frame["fwd_ret"].astype(float)
...
def _scoped_reasons(scope: str, metrics: dict[str, Any], *, require_trades: bool) -> list[str]:
    ...
    if float(metrics["winRate"]) < RECOMPUTED_MIN_WIN_RATE:
        reasons.append(f"{scope}: winRate {metrics['winRate']} < {RECOMPUTED_MIN_WIN_RATE}")
